...
class GraphCreator:
    ...
    def __init__(self, args: Args):
        ...
        self.detections, self.flips, self.detections_probs, self.flips_probs = self.IBMSampler.load_jobdata(verbose=True) # Includes trivial syndromes, size as original file
    ...
    def train_val_split(self, seed=42):
        ...
        self.train_detections_soft, self.train_flips_soft = self.detections_probs[train_idx], self.flips_probs[train_idx] 
        self.val_detections, self.val_flips = self.detections[val_idx], self.flips[val_idx]
        self.val_detections_soft, self.val_flips_soft = self.detections_probs[val_idx], self.flips_probs[val_idx]
        ...
        self.train_detections_soft, self.train_flips_soft = self.train_detections_soft[train_triv_syndrome_mask], self.train_flips_soft[train_triv_syndrome_mask] 
        self.val_detections, self.val_flips = self.val_detections[val_triv_syndrome_mask], self.val_flips[val_triv_syndrome_mask]
        self.val_detections_soft, self.val_flips_soft = self.val_detections_soft[val_triv_syndrome_mask], self.val_flips_soft[val_triv_syndrome_mask]
