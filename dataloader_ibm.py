...
class IBMSampler:
    ...
    def _load_json(self) -> Tuple[List[str], List[str]]:
        ...
        if self.simulated:
            ...
            syndromes, final_states = [], []
            for bitstring, freq in counts.items():
                syndrome, final = bitstring.split()
                syndromes.extend([syndrome] * freq)
                final_states.extend([final] * freq)
        else:
            data = data[0]  # Experimental jobs are returned as a list
            syndromes = data.data.syndrome.get_bitstrings()
            final_states = data.data.final_state.get_bitstrings()

        # Reverse bit order to match IBM's convention
        syndromes = [s[::-1] for s in syndromes]
        final_states = [s[::-1] for s in final_states]

        return syndromes, final_states

    def _get_syndrome_matrix(self, syndromes: List[str], final_states: List[str]) -> np.ndarray:
        """
        ...
        final_syndrome = self._compute_syndrome_differences(final_states)
    ...
    def _extract_logical_flips(self, final_states: List[str]) -> np.ndarray:
        """
        ...
        """
        diff_parity = np.array([s[0] == "1" for s in final_states]) # 0/1 klassning
    ...
    def load_jobdata(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
        syndromes, final_states = self._load_json()
        syndrome_matrix = self._get_syndrome_matrix(syndromes, final_states)
        detection_events = self._extract_detection_events(syndrome_matrix)
        logical_flips = self._extract_logical_flips(final_states)
